      ...
      <div class="col-md-12">
        @if($title_section_exist)
          <x-title-block-headless class="text-center section-heading">
            <x-slot name="title">
              @if($title)
                <{{ $title_type }} class="h2 fw-400 text-blue-500">
                {!! $title !!}
            </{{ $title_type }}>
            @endif
            </x-slot>
            <x-slot name="subtitle">
              @if($subtitle)
                <{{ $subtitle_type }} class="h3 text-blue-600 mb-0">
                {!! $subtitle !!}
            </{{ $subtitle_type }}>
            @endif
            </x-slot>
        ...
        @endif
        <x-logos :logos="$logos" :randomize="$randomize" :disclaimer="$terms" :view-all-mobile="$view_all_mobile ?? ''" :view-less-mobile="str_replace('all', 'less', $view_all_mobile) ?? ''" :headings="$title_section_exist" :wrapper-override="$wrapper_override_url" />
      </div>
    </div>
  </div>
</x-block-theme>
